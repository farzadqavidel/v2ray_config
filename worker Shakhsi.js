        ...
        try {
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                if (路径 == '/') {
                    ...
                    return await config_Json(userID, request.headers.get('Host'), sub, UA, 请求CF反代IP, url, fakeUserID, fakeHostName, env, request);
...
async function config_Json(userID, hostName, sub, UA, 请求CF反代IP, _url, fakeUserID, fakeHostName, env, request) {
    const uuid = (_url.pathname.startsWith(`/${动态UUID}/`)) ? 动态UUID : userID;
    
    // دریافت اطلاعات IP از Cloudflare
    const clientIP = request.headers.get('CF-Connecting-IP') || request.headers.get('X-Forwarded-For') || 'unknown';
    const cf = request.cf || {};
    const countryCode = cf.country || '';
    const city = cf.city || 'نامشخص';
    const country = cf.country || 'نامشخص';
    ...
    // دریافت ISP - از Cloudflare اطلاعات ISP در دسترس نیست، پس از 'نامشخص' استفاده می‌کنیم
    let isp = 'نامشخص';
...
<head>
    ...
    <title id="pageTitle">Give Config</title>
...
<body>
    <div class="container">
        <div class="header">
            <div class="social-links">
                <a href="${atob("aHR0cHM6Ly9naXRodWIuY29tL0ZhcnphZC1HaGF2aWRlbA==")}" target="_blank" class="social-link" title="GitHub">
            ...
            <h1 id="pageHeader">دریافت کانفیگ (F panel)</h1>
        </div>

        <div id="loading" class="loading">
            <div class="spinner"></div>
            <p>در حال بارگذاری اطلاعات پیکربندی...</p>
        </div>
        ...
        <div id="content" class="content">
            ...
            <div class="section details-section">
                ...
                <details id="configLinkDetails">
                    <summary>لینک کانفیگ</summary>
                    <div class="details-content">
                        <div style="margin-bottom: 20px; padding: 16px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid var(--primary-color);">
                            <label for="configName" style="display: block; font-weight: 500; margin-bottom: 8px; color: var(--text-color);">نام خود را وارد کنید:</label>
                            <input type="text" id="configName" placeholder="مثال: MyConfig" style="width: 100%; padding: 12px; border: 2px solid var(--border-color); border-radius: 8px; font-size: 1rem; transition: all 0.3s ease; font-family: 'SF Mono', Monaco, 'Cascadia Code', monospace;" oninput="updateConfigName()">
                            <p style="margin-top: 8px; font-size: 0.85rem; color: var(--text-light);">این نام در کانفیگ vless و clash استفاده می‌شود</p>
                        </div>
                        <div id="linkInfo" style="display: none;">
                            <p id="linkInfoMessage" style="color: var(--text-light); font-style: italic; text-align: center; padding: 20px;">لطفاً ابتدا نام خود را وارد کنید</p>
                        </div>
                    </div>
                </details>
            </div>
    ...
    <div id="qrModal" class="qr-modal">
        <div class="qr-modal-content">
            ...
            <div class="qr-title" id="qrTitle">کد QR</div>
            <div id="qrCode"></div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // بررسی وضعیت نام هنگام باز شدن بخش
            const configLinkDetails = document.getElementById('configLinkDetails');
            if (configLinkDetails) {
                configLinkDetails.addEventListener('toggle', function() {
                    if (this.open) {
                        const nameInput = document.getElementById('configName');
                        if (!nameInput || !nameInput.value.trim()) {
                            updateConfigName(); // نمایش پیام اگر نام وارد نشده باشد
                        }
                    }
                });
            }
        ...
        async function loadConfig() {
            try {
                ...
                await renderConfigInfo();
                // renderLinkInfo را فراخوانی نمی‌کنیم تا زمانی که کاربر نام را وارد کند
                
                // 在页脚显示User-Agent
                document.getElementById('userAgent').textContent = 'User-Agent: ' + configData.UA;
                
            } catch (error) {
                console.error('خطا در بارگذاری پیکربندی:', error);
                document.getElementById('loading').innerHTML = '<p style="color: red;">❌ خطا در بارگذاری پیکربندی، لطفاً صفحه را رفرش کنید</p>';
            }
        }

        async function getIPInfo() {
            try {
                // استفاده از API رایگان ipapi.co (HTTPS)
                ...
                // در صورت خطا، از API جایگزین استفاده می‌کنیم
                try {
                    const response = await fetch('https://ip-api.com/json/?fields=status,message,country,countryCode,city,isp,query');
                    if (response.ok) {
                        const data = await response.json();
                    ...
                    console.error('خطا در API جایگزین:', err);
                }
                return {
                    ip: 'نامشخص',
                    country: 'نامشخص',
        ...
        function getConfigName() {
            const nameInput = document.getElementById('configName');
            return nameInput && nameInput.value.trim() ? nameInput.value.trim() : configData.sub.SUBNAME || 'edgetunnel';
        }

        function updateConfigName() {
            const nameInput = document.getElementById('configName');
            const linkInfo = document.getElementById('linkInfo');
            const linkInfoMessage = document.getElementById('linkInfoMessage');
            const configLinkDetails = document.getElementById('configLinkDetails');
            
            // اگر نام وارد شده باشد
            if (nameInput && nameInput.value.trim()) {
                // مخفی کردن پیام
                if (linkInfoMessage) {
                    linkInfoMessage.style.display = 'none';
                }
                // نمایش لینک‌ها
                if (linkInfo) {
                    linkInfo.style.display = 'block';
                }
                // باز کردن بخش details
                if (configLinkDetails && !configLinkDetails.open) {
                    configLinkDetails.open = true;
                }
                // رندر کردن لینک‌ها
                renderLinkInfo();
            } else {
                // اگر نام خالی باشد
                if (linkInfo) {
                    linkInfo.style.display = 'block'; // نمایش پیام
                }
                if (linkInfoMessage) {
                    linkInfoMessage.style.display = 'block';
                }
            }
        }
        ...
        function renderLinkInfo() {
            ...
            let v2Link = configData.link.v2;
            let clashLink = configData.link.clash;
            
            // دریافت نام از input
            const configName = getConfigName();
            
            // جایگزینی نام در v2 link
            if (v2Link) {
                // جایگزین کردن نام در انتهای لینک (بعد از #)
                v2Link = v2Link.replace(/#[^#]*$/, '#' + encodeURIComponent(configName));
            }
            
            // جایگزینی نام در clash config
            if (clashLink) {
                // جایگزین کردن name: در clash config
                clashLink = clashLink.replace(/name:\s*[^,}]+/, 'name: ' + configName);
            }
            ...
            v2Card.innerHTML = 
                '<div class="link-label">لینک v2</div>' +
            ...
            clashCard.innerHTML = 
                '<div class="link-label">بخش پیکربندی Clash</div>' +
        ...
        async function renderConfigInfo() {
            const container = document.getElementById('configInfo');
            
            // استفاده از اطلاعات IP از config.json
            let ipInfo = configData.ipInfo || {};
            
            // اگر اطلاعات IP در config.json نبود، از API دریافت کنیم
            if (!ipInfo || !ipInfo.ip || ipInfo.ip === 'unknown') {
                container.innerHTML = '<div class="config-card"><div class="config-label">در حال دریافت اطلاعات...</div><div class="config-value">لطفاً صبر کنید</div></div>';
                ipInfo = await getIPInfo();
                ipInfo.flag = getCountryFlag(ipInfo.countryCode);
            }
            
            // ایجاد جدول
            const tableHTML = '<table class="ip-table">' +
                '<thead>' +
                    '<tr>' +
                        '<th>Information</th>' +
                        '<th>Target</th>' +
                ...
                '<tbody>' +
                    '<tr>' +
                        '<td class="ip-info-label">IP</td>' +
                        '<td>' + (ipInfo.ip || 'نامشخص') + '</td>' +
                    '</tr>' +
                    '<tr>' +
                        '<td class="ip-info-label">Country</td>' +
                        '<td>' + (ipInfo.country || 'نامشخص') + ' ' + (ipInfo.flag || '') + '</td>' +
                    '</tr>' +
                '</tbody>' +
            '</table>';
